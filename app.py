...
if st.button("INVOQUER L’ANGE", type="primary"):
    if not refs:
        ...
        if target_frames == n_gen:
            ...
            # downsample (uniforme)
            idx = np.linspace(0, n_gen - 1, target_frames).round().astype(int)
            final_frames = [pil_frames[i] for i in idx]

        else:
            # interpolation crossfade linéaire
            ...
            for i in range(n_gen - 1):
                ...
                for j in range(1, k + 1):
                    ...
                    final_frames.append(PILImage.fromarray(np.clip(mix, 0, 255).astype(np.uint8)))
